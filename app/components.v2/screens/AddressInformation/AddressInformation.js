...
@connect(
  state => ({
    ...
    blacklistedStatesResidency: state.generalData.blacklistedStatesResidency,
  }),
...
class AddressInformation extends Component {
  ...
  validateForm = () => {
    const { formData, actions, blacklistedStatesResidency } = this.props;
    const formErrors = {};
    ...
    if (blacklistedStatesResidency.indexOf(formData.state) !== -1 ) formErrors.state = "We can't work with people from this region!";
