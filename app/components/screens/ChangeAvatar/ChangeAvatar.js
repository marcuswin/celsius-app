...
const images = [
  ...
  },
  {
    url: `${API_URL.replace(
      "/api/v3",
      ""
    )}/profile-images/avatar/cat/profile-cat.png`,
  },
  {
    url: `${API_URL.replace(
      "/api/v3",
  ...
  },
  {
    url: `${API_URL.replace(
      "/api/v3",
      ""
    )}/profile-images/avatar/dog/profile-dog.png`,
  },
  {
    url: `${API_URL.replace(
      "/api/v3",
      ""
    )}/profile-images/avatar/fox/profile-fox.png`,
  },
  {
    url: `${API_URL.replace(
      "/api/v3",
...
class ChangeAvatar extends Component {
  ...
  renderImage = imgSrc => {
    ...
    return (
      <TouchableOpacity
        key={images.indexOf(imgSrc)}
        style={style.avatar}
        onPress={() => this.updateProfilePicture(imgSrc)}
      >
        <View>
    ...
    return (
      <RegularLayout>
        <View style={{ alignItems: "center", marginBottom: 10, height: 130 }}>
          ...
          <CelButton style={{ marginTop: 7 }} onPress={this.goToCamera} basic>
            Take a picture
          </CelButton>
        </View>
        <Separator
        ...
        />
        <ScrollView>
          <View style={style.wrapper}>{images.map(this.renderImage)}</View>
        </ScrollView>
      </RegularLayout>
    );
  }
