...
class BorrowLanding extends Component {
  ...
  applyForAnother = () => {
   ...
   render() {
    ...
    if (kycStatus && kycStatus !== KYC_STATUSES.passed) return  <BorrowCalculator purpose={ EMPTY_STATES.NON_VERIFIED_BORROW }/>;
  ...
  const minLtv = Math.max(...ltv.map(x => x.percent))
   ...
   if (maxAmount < minimumLoanAmount / minLtv) return <BorrowCalculator purpose={EMPTY_STATES.BORROW_NOT_ENOUGH_FUNDS} />
