...
class BorrowLanding extends Component {
    ...
    emitParams = () => {
        ...
        if (formData && formData.ltv) {
            ...
            loanParams.collateralNeeded =
                Number(formData.amount) / currencies.find(c => c.short === formData.coin).market_quotes_usd.price / formData.ltv.percent
            ...
            loanParams.minimumLoanAmountCrypto =
                minimumLoanAmount / currencies.find(c => c.short === eligibleCoins[indexOfLargestAmount].short).market_quotes_usd.price
                    ...
                    <View style={style.buttonIconText}>
                        ...
                        >
                            <View style={style.buttonItself}>
                                ...
                                />
                                <CelText align="center">Apply for</CelText>
                                <CelText align="center">a loan</CelText>
                            </View>
                        </TouchableOpacity>
                        <Separator vertical height={'35%'} top={50} />
                        <TouchableOpacity
                            style={{ marginLeft: widthPercentageToDP('3.3%'), marginRight: widthPercentageToDP('3.3%') }}
                            onPress={() => {
    ...
    renderDefaultView() {
        ...
        const hasEnoughOriginalCoin = !!walletSummary.coins.find(
            coin => coin.short === marginCalls[0].collateral_coin && coin.amount >= marginCalls[0].margin_call_amount
        )
        ...
        return (
            <RegularLayout padding={'20 0 100 0'}>
                <View>
                    <View style={{ marginLeft: 20, marginRight: 20 }}>{this.renderCard()}</View>
                    <Animated.ScrollView
                        horizontal
    ...
    renderIntersection() {
        ...
        if (maxAmount < minimumLoanAmount / minLtv)
            return <BorrowCalculatorScreen emitParams={this.emitParams} purpose={EMPTY_STATES.BORROW_NOT_ENOUGH_FUNDS} />
