...
class BorrowLanding extends Component {
  ...
  render () {
    ...
    if (kycStatus && kycStatus !== KYC_STATUSES.passed) {
      return (
        <StaticScreen
    ...
    if (!user.celsius_member) {
      return (
        <StaticScreen
          emptyState={{ purpose: EMPTY_STATES.NON_MEMBER_BORROW }}
        />
      )
    }
    if (!loanCompliance.allowed) {
    ...
    */
    ...
    let walletCoins =
      walletSummary && walletSummary.coins ? walletSummary.coins : []
    walletCoins = walletCoins.filter(allowedCoin =>
      loanCompliance.coins.includes(allowedCoin.short)
    )
    const arrayOfAmountUsd = walletCoins.map(amount => amount.amount_usd)
    const indexOfLargestAmount = arrayOfAmountUsd.indexOf(
      Math.max(...arrayOfAmountUsd)
    )
    if (!walletCoins || !walletCoins[indexOfLargestAmount]) return <LoadingScreen />
      ...
      return (
        <StaticScreen
          emptyState={{
            ...
            heading:
              Number(largestAmount) !== 0
                ? `To apply for a loan you just need ${
                  Number(largestAmount) !== 0
                    ? formatter.crypto(
                      minimumAmountInCoin / minLtv - largestAmount,
                      largestAmountCoin,
                      { symbol: '' }
                    )
                    : minimumLoanAmount
                } more ${
                  Number(largestAmount) !== 0 ? largestAmountCoin : 'USD'
                }`
                : `To apply for a loan you just need $10.000 in BTC`,

            paragraphs: [
              `The current loan minimum is ${formatter.usd(
                minimumLoanAmount
              )}. We are working hard on enabling smaller loans. Until we make it happen, you may want to deposit more coins and enable this service immediately.`
            ],
            onPress: () =>
              kycStatus === KYC_STATUSES.passed
          ...
          <View>
            ...
            {allLoans &&
              allLoans.map(loan => {
                ...
                return (
                  <Card key={loan.id}>
                    <CelText type='H6' weight='500' margin={'0 0 0 0'}>
                      Your loans
                    </CelText>
                    <TouchableOpacity
                      style={{
                    ...
                    >
                      <View
                        style={[
                          style.iconWrapper,
                          { backgroundColor: loanStatusDetails.color }
                        ]}
                      >
                        <Icon
                      ...
                      <View style={style.info}>
                        ...
                        <View>
                          ...
                          <CelText color={loanStatusDetails.color}>
                            {loanStatusDetails.displayText}
                          </CelText>
                        </View>
                      </View>
                    </TouchableOpacity>
