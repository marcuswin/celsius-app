...
class TransactionConfirmation extends Component {
  ...
  isConfirmButtonDisabled = (withdrawalAddress) => {
    ...
    if (withdrawalAddressValue && formData.currency === "xrp") {
      return !formData.hasTagValue && !formData.coinTag;
    }

    if (withdrawalAddressValue && formData.currency === "xlm") {
      return !formData.hasMemoIdValue && !formData.coinMemoId;
    }
