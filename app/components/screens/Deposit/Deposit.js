...
class Deposit extends Component {
  ...
  fetchAddress = async currency => {
    ...
    // }
    if (!walletAddresses[`${currency}Address`]) await actions.getCoinAddress(currency)
        ...
        {address && !isFetchingAddress ? (
          <View style={styles.container}>
            ...
            <CelText margin={"20 0 20 0"} align={"center"} color={STYLES.COLORS.CELSIUS_BLUE} type={"H4"} weight={"300"}
                     onPress={() => Linking.openURL(link)}>{cryptoUtil.provideText(formData.selectedCoin)}</CelText>
            }
