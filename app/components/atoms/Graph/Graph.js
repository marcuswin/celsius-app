...
class Graph extends React.Component {
  ...
  yRange = [height - verticalPadding, verticalPadding ];
  yDomain = [0,  Math.max(...this.yValue)];
  xRange = [0, width];
  ...
  line = d3.shape.line().x(d => this.scaleX(d.x)).y(d => this.scaleY(d.y)).curve(d3.shape.curveBasis)(data);
  ...
  moveCursor(value) {
    ...
    this.labelText.current.setNativeProps({ text: formatter.round(this.scaleY.invert(y)) });
    if ( x <= width / x) {
        ...
        <View style={{
          ...
          <View>
            <Svg {...{ width, height }}>
              ...
              <Path d={this.line} stroke={"#367BE2"} strokeWidth={5} fill="transparent"/>
            ...
            {data.map((date, index) => (
              <CelText key={date.label} type={"H6"}
                       style={{
                         ...
                         left: this.scaleX(date.x) - widthPercentageToDP("3%"),
