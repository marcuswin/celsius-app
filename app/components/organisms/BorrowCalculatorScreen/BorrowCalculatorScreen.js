    ...
    switch (purpose) {
      case EMPTY_STATES.NON_VERIFIED_BORROW:
        return {
          ...
          bottomParagraph: loanParams ? `Verify your identity to start using your coins as collateral and get a dollar loan starting at just ${formatter.percentageDisplay(loanParams.bestLtv)} APR` : null,
      ...
      case EMPTY_STATES.BORROW_NOT_ENOUGH_FUNDS:
        return {
          ...defaultProps,
          subtitle: '',
          bottomHeading: loanParams ? `To apply for a loan, you need to deposit an additional ${formatter.crypto(loanParams.missingCollateral, loanParams.largestShortCrypto)}`:null,
          bottomParagraph: 'Deposit more coins to start your first loan application',
          buttonCopy: 'Deposit coins',
          onPress: () => actions.navigateTo("Deposit", { coin: loanParams ? loanParams.largestShortCrypto : null }),
        }
